...
//(public|protected|private|static|\s) +([\w\<\>\[\]]|\w*\<[\w ]*\,?[\w ]*\>)+\s+(\w+) *\([^\)]*\) *(\{?|[^;]) ?[throws]?[\w \,]*{
//(public|protected|private|static|\\s) +([\\w\\<\\>\\[\\]]|\\w*\\<[\\w ]*\\,?[\\w ]*\\>)+\\s+(\\w+) *\\([^\\)]*\\) *(\\{?|[^;]) ?[throws]?[\\w \\,]*{
//loc = [ ^\@\.\(]([\w ]*\s?=?\s?[\w\+ \<\>\(]* ?(\;|\)|\:|{)|else)
...
public class ProcessadorArquivosJava implements IProcessadorArquivos {
	...
	private static final String patternMethods = "(public|protected|private|static|\\s) +([\\w\\<\\>\\[\\]]|\\w*\\<[\\w ]*\\,?[\\w ]*\\>)+\\s+(\\w+) *\\([^\\)]*\\) *(\\{?|[^;]) ?[throws]?[\\w \\,]*\\{";
	...
	private int CalculaLocTexto(String texto){
		Pattern p = Pattern.compile(patternLoc);
		Matcher m = p.matcher(texto);
		int i = 0;
		while (m.find()) i++;
		return i;
	...
	private void CalculaLoc(List<IParte> partes){
		for (IParte parte : partes) {
			parte.setTamanho(CalculaLocTexto(parte.getTexto()));
			if (parte.getPartes().size() > 0){
				CalculaLoc(parte.getPartes());
			}
		}
	}	
	...
	public void processaArquivos(IFilaArquivos arquivos) throws FileNotFoundException, IOException {
		...
		while (!arquivos.eof()) {
			...
			for (Entry<String, String> entry : ret.entrySet()) {
				arq.getPartes().add(_factArquivo.createParte(entry.getKey(), entry.getValue(), PartType.Classe));			
			}
			
			// processa partes separando os métodos
			for (IParte parte : arq.getPartes()) {
				Map<String, String> methods = extractMethod(parte.getTexto());
				for (Entry<String, String> entry : methods.entrySet()) {
					parte.getPartes().add(_factArquivo.createParte(entry.getKey(), entry.getValue(), PartType.Metodo));			
				}				
			}
			
			CalculaLoc(arq.getPartes());			
		}
	}
